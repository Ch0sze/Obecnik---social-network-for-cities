...
{
    public static string ValidationClass<TModel, TProperty>(this ViewDataDictionary<TModel> viewData,
        Expression<Func<TModel, TProperty>> selector)
    {
        return viewData.ValidationClass(((MemberExpression)selector.Body).Member.Name);
    }

    public static string ValidationClass<T>(this ViewDataDictionary<T> viewData, string key)
    {
        return viewData.ModelState.GetValidationState(key) switch
        {
